    ...
    dispatch_async(self.sessionQueue, ^{
        ...
        if ([_currentDevice lockForConfiguration:&error]) {
            if ([_currentDevice isFocusPointOfInterestSupported]) {
                NSLog(@"returning to default AVCaptureFocusModeContinuousAutoFocus");
                
                CGPoint defaultPoint = CGPointMake(.5f, .5f);
                
                [_currentDevice setFocusPointOfInterest:defaultPoint];
            ...
            NSLog(@"sessionManager - %@", [error localizedDescription]);
        }
    });
}
...
- (void) setupSession:(NSString *)defaultCamera
{
    ...
    dispatch_async(self.sessionQueue, ^{
        ...
        [[NSNotificationCenter defaultCenter] removeObserver:self name:AVCaptureDeviceSubjectAreaDidChangeNotification object:_currentDevice];
        ...
        _currentDevice = videoDevice;
        
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(subjectAreaDidChange:) name:AVCaptureDeviceSubjectAreaDidChangeNotification object:_currentDevice];
